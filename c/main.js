...
function initFrameDifferencing(){
	...
	rt1 = new THREE.WebGLRenderTarget(w, h, { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBFormat });
	...
	rt2 = new THREE.WebGLRenderTarget(w, h, { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBFormat });
	...
	rtDiff = new THREE.WebGLRenderTarget(w, h, { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBFormat });
	...
	rtFB = new THREE.WebGLRenderTarget(w, h, { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBFormat });
	...
	rtFB2 = new THREE.WebGLRenderTarget(w, h, { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBFormat });
	...
	rtBump = new THREE.WebGLRenderTarget(w, h, { minFilter: THREE.LinearFilter, magFilter: THREE.LinearFilter, format: THREE.RGBFormat });
...
function many(){
   ...
   for(var j = -canvas.height; j < canvas.height*2; j+=distY){
        ...
        bezierY(0,j, canvas.width, j, "#ffffff" /*hslaColor(j/5, 100, 50, alpha)*/);  
    }
    for(var i = -canvas.width; i < canvas.width*2; i+=distX){
    	...
    	var color = "rgba("+r+","+g+", "+b+", "+alpha+")";
        bezierX(i, 0, i, canvas.height, "#ffffff" /*hslaColor(i/5, 100, 50, alpha)*/);  
...
function draw(){
    ...
    materialBump.uniforms.time.value = time;
	many();
