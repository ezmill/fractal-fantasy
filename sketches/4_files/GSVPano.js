...
GSVPANO.PanoLoader = function(parameters) {
	'use strict';
	var _parameters = parameters || {}, _location, _zoom, _panoId, _panoClient = new google.maps.StreetViewService(),
		_count = 0,
		_total = 0,
		_canvas = [],
		_ctx = [],
	...
	this.adaptTextureToZoom = function() {
		...
		for (var y = 0; y < _hc; y++) {
			for (var x = 0; x < _wc; x++) {
				...
				else c.width = w - (maxW * x); if (y < (_hc - 1)) c.height = maxH;
			...
			for (var x = 0; x < w; x++) {
				...
				(function(x, y) {
					if (_parameters.useWebGL) {
						...
						var texture = GL.Texture.fromURL(url)
						...
						img.addEventListener('load', function() {
							var texture = GL.Texture.fromImage(img)
							self.composeFromTile(x, y, texture);
						});
					}
				})(x, y);
